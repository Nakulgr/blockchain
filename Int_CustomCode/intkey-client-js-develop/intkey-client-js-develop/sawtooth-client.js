...
const SawtoothClientFactory = (factoryOptions) => {
	...
	console.log('factoryOptions  restApiUrl ---- ', factoryOptions.restApiUrl)
  return {
    async get(url) {
      try {
    ...
    newTransactor(transactorOptions) {
      ...
      return {
        async post(payload, txnOptions) {
          ...
          // Post the batch list
          //console.log('posting batch list   data is ---- '+batchListBytes)
          try {
            const res = await axios({
              method: 'post',
              baseURL: factoryOptions.restApiUrl,
            ...
            console.log('error when submitting to rest-api ', err)
          }
        }
      }
    }
